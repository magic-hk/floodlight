...
public class DeviceManagerImpl implements IDeviceService, IOFMessageListener, ITopologyListener, IFloodlightModule, IEntityClassListener, IInfoProvider {
	...
	protected Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		...
		/**
		 * 修改,这里用来删除不必要的host
		 * 这里暂时解决不了,只能通过先启动mininet,等到它稳定以后,再启动控制器就不会有这个问题了
		 */
//		if(eth.getEtherType() == EthType.ARP) {
//			return Command.CONTINUE;
//		}这么做会存在问题,即ip包也过滤掉了
		//logger.info("-------------eth:{}",eth.getParent());
		
		
		NodePortTuple npt = new NodePortTuple(sw.getId(), inPort);
		MacAddress gatewayMac = null;
		if (routingService.isL3RoutingEnabled()) {//判断是否为三层路由
			Optional<VirtualGatewayInstance> instance = gatewayService.getGatewayInstance(sw.getId());
			if (!instance.isPresent()) {//获取到的当前虚拟网关为空，尝试利用npt再获取一次
		...
		// Extract source entity information 提取源实体信息
		...
		//从ARP包学习特殊的VRRP设置。在VRRP设置中，源MAC地址和发送者MAC地址可以不同。
		//在这种情况下，我们需要学习VRRP IP地址的IP到MAC映射。源实体不会拥有该信息。因此，在这种情况下，需要单独调用来学习设备。
		...
		//在SRC_DEVICE上下问中存储source设备
		fcStore.put(cntx, CONTEXT_SRC_DEVICE, srcDevice);//会记录包从哪里进来,然后在后面的模块中使用
	...
	protected Device learnDeviceByEntity(Entity entity) {
		...
		// starvation loops
		//如果检测到并发修改，可能需要重新启动学习过程。注意，我们确保至少有一个线程总是成功的，这样我们就不会陷入无限的饥饿循环
		while (true) {
			deviceUpdates = null;
			...
			if (deviceKey != null) {
				...
				//如果辅助索引不包含实体，则创建包含实体的新设备对象，如果实体位于附件点端口，则生成新设备ID。否则忽略。
