...
public class LinkInfo {
	...
	//获取一个平均的历史时延，这里是窗口时延的平均，默认的时延窗口为10
	private U64 getLatencyHistoryAverage() {
		if (!isLatencyHistoryFull()) {//判断，延迟窗口是否被填充满，
			return null;
	...
	private U64 getLatency() {
		...
		if (newLatency != null) {
			...
			if ((((double) Math.abs(newLatency.getValue() - currentLatency.getValue())) 
					...
					>= latencyUpdateThreshold) {//时延的波动是否大于阈值，这里的阈值是0.50，如果是的话，更新当前的时延
				/* perform update */
	 ...
	 * 向列表追加一个新的(大概是最近的)延迟。
	 * 如果这是执行的第一次延迟更新，则设置当前延迟。注意，后者用作延迟初始化器。
	 * 
	 * @param latency
