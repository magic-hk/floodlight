...
public interface IRoutingService extends IFloodlightService {
   ...
   enum PATH_METRIC {
        ...
        //新增的自己的指标
 	    RESIDUAL_BANDWIDTH("residual_bandwidth");//剩余带宽,可用带宽
    ...
    */
   enum GET_MODE{
	   CACHE("cache"), //只尝试从cache中获取数据
	   ELASTIC_COMPUTE("elastic_compute"),//尝试从cache中获取数据，当cache不名中的时候，在计算路径
       FORCE_COMPUTE("force_compute");//强制计算路径
       String name;
	   private GET_MODE(String s) {
           name = s;  
       }    
   }
    /**
     ...
     * @return
     */
    List<Path> getMetricsConstraintPathsSlow(DatapathId src, DatapathId dst, int numReqPaths,Map<QOS_METRIC,MetricConstraint> metrics);
    List<Path> getMetricsConstraintPathsFast(DatapathId src, DatapathId dst, int numReqPaths,Map<QOS_METRIC,MetricConstraint> metrics);
    
    /**
     * 一个统一的路由服务的指标、获取模式相关的路径获取接口
     * @param src
     * @param dst
     * @param numReqPaths
     * @param metricsContriants 指标约束集合
     * @param getMode 获取路径的模式
     * @return
     */
    List<Path> getMetricsConstraintPaths(DatapathId src, DatapathId dst, int numReqPaths,FlowDemandMetricConstraints metricsContriants,GET_MODE getMode);

	Path getMetricsConstraintPath(DatapathId src, OFPort srcPort, DatapathId dst, OFPort dstPort,
