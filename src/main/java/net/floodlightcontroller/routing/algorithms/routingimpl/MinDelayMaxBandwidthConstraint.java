...
public class MinDelayMaxBandwidthConstraint extends RoutingAlgorithmBase {
	
	//是否注册,根据该属性，在加载的时候，可以选择性的注册
	private static final boolean WANT_TO_REGISTER = true;
	
	//单例模式
	private static  MinDelayMaxBandwidthConstraint instance = null;
	
	//实例构造，在这里面添加算法的类名，和支持的指标
	private MinDelayMaxBandwidthConstraint() {
		super();
		 ...
		 * 这里可能会出现覆盖的情况
		 */
		MetricConstraint mcFirst = new MetricConstraint(QOS_METRIC.LATENCY,CONSTRAINT.PATH_MIN,1);
		MetricConstraint mcSecond = new MetricConstraint(QOS_METRIC.RESIDUAL_BANDWIDTH,CONSTRAINT.PATH_MAX,0);
	...
	public static MinDelayMaxBandwidthConstraint getInstance() {
		if(instance == null) {
			return new MinDelayMaxBandwidthConstraint();
		}
		return instance;
	}
