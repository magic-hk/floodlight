...
import net.floodlightcontroller.routing.IRoutingService.GET_MODE;
...
public class TopologyInstance {
    ...
    //private Map<PathId, List<Path>>				merticPathcache;
    
    private Map<PathId, List<Path>>				merticPathcaches;
    ...
    protected TopologyInstance(Map<DatapathId, Set<OFPort>> portsWithLinks,
        ...
        //this.merticPathcache = new HashMap<PathId, List<Path>>();
     ...
     * 
     * @param parentIndex: DFS index of the parent node 父节点的DFSID
    ...
    private class NodeWithMetrics implements Comparable<NodeWithMetrics> {
        private final DatapathId node;
        public DatapathId getNode() {
            return node;
        }
       ...
       private final Map<QOS_METRIC,U64> constraints;
       public Map<QOS_METRIC,U64> getConstrains() {
           return constraints;
       }
       
      
        public NodeWithMetrics(DatapathId node,int weight, Map<QOS_METRIC,U64> constraints) {
            this.node = node;
            this.weight = weight;
            this.constraints = constraints;
        }
        
        public NodeWithMetrics(DatapathId node,int weight) {
            this.node = node;
            this.weight = weight;
            this.constraints = new HashMap<QOS_METRIC,U64>();
        }
        
        /**
         * 当要考虑并记录多个指标的时候，可以和NodeWithMetrics(DatapathId node,int weight)构造函数结合使用
         * @param metric
         * @param value
         */
        public void addMetric(QOS_METRIC metric,U64 value) {
        	this.constraints.put(metric, value);
        }
        /**
         * 
         * @param node 交换机id
         ...
         * @param value 指标值
         */
        public NodeWithMetrics(DatapathId node,int weight, QOS_METRIC metric,U64 value) {
            this.node = node;
            this.weight = weight;
            this.constraints = new HashMap<QOS_METRIC,U64>();
            constraints.put(metric, value);
        }
        
        //将权重作为比较基础
        @Override
        public int compareTo(NodeWithMetrics o) {
            if (o.weight == this.weight) {//如果dist相同，则比较
                return (int)(this.node.getLong() - o.node.getLong());
            }
        ...
        public boolean equals(Object obj) {
            ...
            if (getClass() != obj.getClass())
                return false;
            NodeWithMetrics other = (NodeWithMetrics) obj;
            if (!getOuterType().equals(other.getOuterType()))
                return false;
            if (node == null) {
                if (other.node != null)
...
//                    log.info("All switches: {}", switches);
//                    log.info("linksExternal: {}", linksExternal);
//                    log.info("portsWithLinks: {}", portsWithLinks);
...
//                    log.info("---------Dijkstra Test End----------");
     ...
     * @return
     */
    private Map<NodePortTuple, Set<Link>> prunedWithBandWidth(long value){
    	  Map<NodePortTuple, Set<Link>> prunedLinks = new HashMap<NodePortTuple, Set<Link>>();
    	 //进行状态信息统计，感觉这个可能有点费时间
    	 TopologyManager.statisticsService.collectStatistics(true);
         log.debug("prune link with bandwidth");
         //遍历所有的交换机
         for (DatapathId s : switches) {
             if (portsWithLinks.get(s) == null) continue;
             //获取当前交换机上，有link相连接的端口，并遍历
             for (OFPort p : portsWithLinks.get(s)) {
            	 //当前遍历到的交换机、端口对应的元组
                 NodePortTuple npt = new NodePortTuple(s, p);
                 //对应的link是否存在
                 if (links.get(npt) == null) continue;
                 for (Link l : links.get(npt)) {//遍历与该端口相连接的link
                	//判断sw存在,考虑当link双向问题，这里是以传输计算的，所以必须要是源
                     if (switches.contains(l.getSrc()) && switches.contains(l.getDst())) {
                         ...
                         if (spb != null) {
                             ...
                             linkSpeed = spb.getLinkSpeedBitsPerSec().getValue();
                            if((linkSpeed-bpsTx)<=0) {//当可用带宽很小的时候，则link的权重取最大
                            	linkWeight=MAX_LINK_WEIGHT;
                            }else {
                            	linkWeight=new Long(bpsTx/(linkSpeed-bpsTx)).intValue();//权重公式：已经使用的带宽/剩下的带宽
                            }
                            if(linkWeight>MAX_LINK_WEIGHT) {
                            	linkWeight=MAX_LINK_WEIGHT;
                            }
                         }
//                         log.info("---- truely linkSpeed {}",linkSpeed);
//                         log.info("---- truely RESIDUAL_BANDWIDTH {}",(linkSpeed-bpsTx));
                         //这里的单位都是bit/s
                         if( (linkSpeed-bpsTx) <value) {//剩余带宽不能满足需求的时候
                        	 continue;
                         }else {
//                        	 if(linkWeight == 0) {//如果使用流量没有超过50%的时候，退化成按跳数
//                        		 linkWeight++;
//                        	 }
                        	 
                        	 //为l设置权重
                        	 l.setWeight(linkWeight);
                        	 if (prunedLinks.containsKey(npt)) {//如果已经在map中存在了，添加新发现的link到set中
                            	 prunedLinks.get(npt).add(l);
                             } else {
                            	 prunedLinks.put(npt, new HashSet<Link>(Arrays.asList(l)));
                             }
                         }
    ...
    private List<Path> yens(DatapathId src, DatapathId dst, Integer K, Archipelago aSrc, Archipelago aDst,Map<QOS_METRIC,MetricConstraint> metrics) {
        log.debug("YENS ALGORITHM -----------------");
        log.debug("Asking for paths from {} to {}", src, dst);
        log.debug("Asking for {} paths", K);
        //约束带宽值
        long bandwidth = (metrics!=null&&metrics.get(QOS_METRIC.RESIDUAL_BANDWIDTH)!=null)? metrics.get(QOS_METRIC.RESIDUAL_BANDWIDTH).getValue().getValue(): 0;
        
        //log.info("---mertic RESIDUAL_BANDWIDTH {} bit/s",bandwidth);
        
        //用带宽减枝,并且给为每个link计算了其带宽权重=已经使用的带宽/剩下的带宽
        Map<NodePortTuple, Set<Link>> prunedLinks = prunedWithBandWidth(bandwidth);
        
        Map<DatapathId, Set<Link>> prunedLinkDpidMap = buildLinkDpidMap(switches, portsWithLinks, prunedLinks);
       
        //log.info("---prunedLinkDpidMap {}",prunedLinkDpidMap);
       
        Map<DatapathId, Set<Link>> copyOfLinkDpidMap = new HashMap<DatapathId, Set<Link>>(prunedLinkDpidMap);
        ...
        BroadcastTree bt = dijkstraDCBC(copyOfLinkDpidMap, dst, metrics, true);
        ...
        for (int k = 1; k < K; k++) {
            ...
            for (int i = 0; i <= A.get(k - 1).getPath().size() - 2; i = i + 2) {
                ...
                Map<NodePortTuple, Set<Link>> allLinksCopy = new HashMap<NodePortTuple, Set<Link>>(prunedLinks);
                ...
                Path spurPath = buildPath(new PathId(spurNode, dst), dijkstraDCBC(copyOfLinkDpidMap, dst, metrics, true));
            ...
            log.debug("--------------------------------------------");
            Path shortestPath = removeShortestPath(B);//这里默认用link的带宽权重来进行计算
            log.debug("--------------AFTER------------------------");
            for (Path r : B) {
                log.debug(r.toString());
            }
    ...
    private BroadcastTree dijkstraDCBC(Map<DatapathId, Set<Link>> links,DatapathId root,Map<QOS_METRIC,MetricConstraint>  mertics,boolean isDstRooted) {
        HashMap<DatapathId, Link> nexthoplinks = new HashMap<DatapathId, Link>();
        //记录root到DatapathId指定的sw的cost
        HashMap<DatapathId, Integer> cost = new HashMap<DatapathId, Integer>();
    
   
        long delayConstraint = (mertics!=null&&mertics.get(QOS_METRIC.LATENCY)!=null)? mertics.get(QOS_METRIC.LATENCY).getValue().getValue(): 10000;
       // log.info("---mertic LATENCY {}",delayConstraint);
        ...
        //其存储上不是有序的，但是根据指定方式来出队列，它是有序的
        //这里的话是按dist来排序
        PriorityQueue<NodeWithMetrics> nodeq = new PriorityQueue<NodeWithMetrics>();
        
        //向优先队列中插入元素,根节点权重和时延迟都为0
        nodeq.add(new NodeWithMetrics(root,0,QOS_METRIC.LATENCY,U64.of(0)));
        cost.put(root, 0);
        ...
        while (nodeq.peek() != null) {
        	//获取权重最小的节点
        	NodeWithMetrics n = nodeq.poll();
            DatapathId cnode = n.getNode();
            int cweight = n.getWeight();//累积权重
            long cdelay = n.getConstrains().get(QOS_METRIC.LATENCY).getValue();//累积时延
            if (cweight >= MAX_PATH_WEIGHT) break;//最小权重都超大的情况
            if (seen.containsKey(cnode)) continue;
            seen.put(cnode, true);
            //log.debug("cnode {} and links {}", cnode, links.get(cnode));
            if (links.get(cnode) == null) continue;
...
//                log.info("Neighbor cost: {}", cost.get(neighbor));
                ...
                if (seen.containsKey(neighbor)) continue;      
                //计算到邻居的距离
                int ndweight = cweight + link.getWeight(); // the weight of the link, always 1 in current version of floodlight.
              
                //更新时延
                long ndelay = cdelay+link.getLatency().getValue();
...
//                    log.info("---------Dijkstra Test End----------");
                ...
                //如果新计算的ndist的值，小于cost中记录的先前root到neighbor的cost
                if (ndweight < cost.get(neighbor)) {//权重满足更新要求
                	Path leastDelayPath = null;
                	if (isDstRooted == true) {//判断方向，因为传进来的有时候是目的交换机sw。如果算单路径其实是没有必要这么算，这里主要是为了生成广播树
                		leastDelayPath=getPath(neighbor,root);
                    } else {
                    	leastDelayPath=getPath(root,neighbor);
                    }
                	long oldleastDelay =0;
                	if(leastDelayPath!=null) {
                		oldleastDelay = leastDelayPath.getLatency().getValue();
                	}
                	
                	//利用前面计算的最短时延来判断，最后结果是否可以满足时延要求，如果可以则更新距离
                	if(leastDelayPath.getHopCount()>0&&ndelay+oldleastDelay<=delayConstraint) {
                		cost.put(neighbor, ndweight);
                        //邻居节点，通过哪个link连接到邻居节点
                        nexthoplinks.put(neighbor, link);
                        //新建NodeWithMetrics，来更新距离
                        NodeWithMetrics ndTemp = new NodeWithMetrics(neighbor, ndweight, QOS_METRIC.LATENCY,U64.of(ndelay));
                        //根据id来删除
                        nodeq.remove(ndTemp);
                        // add the current object to the queue.
                        nodeq.add(ndTemp);//添加完成后，会重新排序，保证dist最小的在队列首部
     ...
     * 如果源路由和目标路由存在于ROUTECACHE中，该函数返回源路由和目标路由之间的K个数。
     * 如果用户请求的路由多于可用的，则只返回已经存储在内存中的路由。这个值可以在floodlight默认值中进行调整。
     *
     ...
     * 此函数返回源和目标之间的K条路径。它将尝试从pathcache中检索这些路径。如果用户请求的路径多于存储的路径，Yen的算法将使用传入的K值运行。
     *
     ...
     * @return
     */
    public List<Path> getMerticsConstraintPathsFast(DatapathId src, DatapathId dst, int k,Map<QOS_METRIC,MetricConstraint> mertics) {
        PathId routeId = new PathId(src, dst);
        List<Path> routes =new ArrayList<Path>(); //merticPathcache.get(routeId);//这个cache还没想好哪里加
    ...
    public List<Path> getMerticsConstraintPathsSlow(DatapathId src, DatapathId dst, int k,Map<QOS_METRIC,MetricConstraint> mertics) {

    	PathId pathId = new PathId(src, dst);
//        List<Path> paths = merticPathcache.get(pathId);
//
//        if (paths == null || k < 1) return ImmutableList.of();
//        
//        if (k >= TopologyManager.getMaxPathsToComputeInternal() || k >= paths.size()) {
//        	log.info("-----runing mertic yens-------");
//            return yens(src, dst, k, getArchipelago(src), getArchipelago(dst),mertics); /* heavy computation */
//        }
//        else {
//            return new ArrayList<Path>(paths.subList(0, k));
//        }
    	return yens(src, dst, k, getArchipelago(src), getArchipelago(dst),mertics);
    }
    ...
    public void setPathLatency(Path p) {
        U64 cost = U64.ZERO;
    ...
    private Path removeShortestPath(List<Path> routes) {
        log.debug("REMOVE SHORTEST PATH -------------");
        // If there is nothing in B, return
        if(routes == null){
            log.debug("Routes == null");
        ...
        for (Path r : routes) {
            ...
            // TODO Get the path cost from the route object
            for (NodePortTuple npt : r.getPath()) {
                if (links.get(npt) == null || links.get(npt).iterator().next().getWeight() == null) {
                    pathCost++;
                }
                else {
                    pathCost += links.get(npt).iterator().next().getWeight();
                }
            }
            log.debug("Path {} with cost {}", r, pathCost);
            // If it is smaller than the current smallest, replace variables with the path just found
