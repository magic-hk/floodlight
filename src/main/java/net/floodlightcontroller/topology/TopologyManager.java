 ...
 * 拓扑管理器负责维护控制器对网络图的概念，以及通过拓扑查找路由的实现工具。
 */
...
ITopologyManagerBackend, ILinkDiscoveryListener, IOFMessageListener {
     ...
     * 重新计算拓扑的线程，这个线程一直运行，然而applyUpdates()函数有一个阻塞调用
     */
    ...
    public boolean isAttachmentPointPort(DatapathId switchid, OFPort port) {
    	...
    	//如果交换机端口是“tun-bsn”端口，那么它就不是连接点端口，而不管是否通过它找到链接。
        ...
        if (ti.isAttachmentPointPort(switchid, port) == false)//排除link节点
            return false;
    ...
    public void startUp(FloodlightModuleContext context) {
        ...
        if (role != HARole.STANDBY) {//每隔500MS执行一次，新建拓扑实例任务
            newInstanceTask.reschedule(TOPOLOGY_COMPUTE_INTERVAL_MS, TimeUnit.MILLISECONDS);
        }
...
//            log.info("----------HK TEST---------");
     ...
     * 
     * 未处理关于switch断开和端口关闭的更新。
     * LinkDiscoveryManager预计将处理这些消息并发送多个被删除的链接消息。
     * 然而，LinkDiscoveryManager中的所有更新都将被传播到拓扑的监听器中。
     */
     ...
     * 这个函数计算一个新的拓扑实例。
     * 它忽略了连接到所有广播域端口和隧道端口。这个方法返回是否一个新实例被创建。
     */
    protected boolean createNewInstance(String reason, boolean forced) {
        ...
        // Identify all broadcast domain ports. 标识所有的广播域端口（非OF端口）
        // Mark any port that has inconsistent set of links
        // as broadcast domain ports as well.
        //将具有不一致链接集的任何prot标记为广播域端口。
        //即同一端口链接数大于2，或是多跳链路(非OF区域)
        ...
        // Remove all tunnel links.移除所有的隧道links
        ...
        //switchPorts只包含链接的一部分端口。计算广播端口需要设置所有端口
        ...
        TopologyInstance nt = new TopologyInstance(switchPorts,//有link的sw端口，包含广播域端口和隧道端口
                blockedPorts,//阻塞端口
                openflowLinks,//移除所有的隧道端口和广播域端口相连的links的links
                broadcastDomainPorts,//广播域端口
                tunnelPorts,//隧道端口
                switchPortLinks,//有link的sw端口的links
                allPorts,//所有交换机的所有端口。
                interClusterLinks);//非OF端口相连的inks
     ...
     *  我们希望每个交换机端口最多有两个链路。
     *  这两个链路都必须是连接到同一交换机端口的单向链路。如果没有,我们将标志着这是一个广播域端口。
     */
    ...
    private void addLinkToStructure(Map<NodePortTuple, Set<Link>> s, Link l) {
         ...
         * never be accepted.
         * 因为我们没有把latency加入到.equals()里面，因此，我们需要，将当前已经存在的link移除，以便来更新时延
         */
        s.get(n1).remove(l);
        s.get(n2).remove(l);
        s.get(n1).add(l);
